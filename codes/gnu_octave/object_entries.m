...
% There's no JavaScript-like Object.entries() in GNU Octave.
% But, we can create our own function to mimic it in GNU Octave.

function objectentriesresult = objectentries(anobject)
    % JavaScript-like Object.entries() function
    newarray = {};
    objectkeys = fieldnames(anobject);
    for objectentryindex = (1:1:numel(objectkeys)) % (start:step:stop)
        objectkey = objectkeys{objectentryindex};
        objectvalue = anobject.(objectkey);
        newarray{end + 1} = {objectkey, objectvalue};
    end
    objectentriesresult = newarray;
end

sprint(sprintf("\n"), "% JavaScript-like Object.entries() in GNU Octave struct");
