...
void main() {
	...
	print("JSON_OBJECT?.foo?.bar: ${optionalChainingV1(JSON_OBJECT, ["foo", "bar"])}");
	// JSON_OBJECT?.foo?.bar: baz

	print("JSON_OBJECT?.foo?.baz: ${optionalChainingV1(JSON_OBJECT, ["foo", "baz"])}");
	// JSON_OBJECT?.foo?.baz: null

	print("JSON_OBJECT?.fruits?.[2]: ${optionalChainingV1(JSON_OBJECT, ["fruits", 2])}");
	// JSON_OBJECT?.fruits?.[2]: banana

	print("JSON_OBJECT?.fruits?.[5]: ${optionalChainingV1(JSON_OBJECT, ["fruits", 5])}");
	// JSON_OBJECT?.fruits?.[5]: null

	print('# using JavaScript-like Optional Chaining Operator (?.) function "optionalChainingV2"');

	print("JSON_OBJECT?.foo?.bar: ${optionalChainingV2(JSON_OBJECT, ["foo", "bar"])}");
	// JSON_OBJECT?.foo?.bar: baz

	print("JSON_OBJECT?.foo?.baz: ${optionalChainingV2(JSON_OBJECT, ["foo", "baz"])}");
	// JSON_OBJECT?.foo?.baz: null

	print("JSON_OBJECT?.fruits?.[2]: ${optionalChainingV2(JSON_OBJECT, ["fruits", 2])}");
	// JSON_OBJECT?.fruits?.[2]: banana

	print("JSON_OBJECT?.fruits?.[5]: ${optionalChainingV2(JSON_OBJECT, ["fruits", 5])}");
	// JSON_OBJECT?.fruits?.[5]: null
}
