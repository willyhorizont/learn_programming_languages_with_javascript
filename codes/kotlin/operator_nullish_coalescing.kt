...
fun main() {
    ...
    println("JSON_OBJECT?.foo?.bar: ${(JSON_OBJECT["foo"] as MutableMap<String, Any?>)["bar"] ?: "not found"}")
    // JSON_OBJECT?.foo?.bar ?? "not found": baz

    println("JSON_OBJECT?.foo?.baz: ${(JSON_OBJECT["foo"] as MutableMap<String, Any?>)["baz"] ?: "not found"}")
    // JSON_OBJECT?.foo?.baz ?? "not found": not found

    println("JSON_OBJECT?.foo?.bar: ${prettyJsonStringify((JSON_OBJECT["foo"] as MutableMap<String, Any?>)["bar"] ?: "not found")}")
    // JSON_OBJECT?.foo?.bar ?? "not found": "baz"

    println("JSON_OBJECT?.foo?.baz: ${prettyJsonStringify((JSON_OBJECT["foo"] as MutableMap<String, Any?>)["baz"] ?: "not found")}")
    // JSON_OBJECT?.foo?.baz ?? "not found": "not found"
}
