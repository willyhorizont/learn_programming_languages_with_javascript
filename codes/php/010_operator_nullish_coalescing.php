...
console_log(string_interpolation('(JSON_OBJECT?.foo?.bar ?? "not found") or (JSON_OBJECT?.["foo"]?.["bar"] ?? "not found"): ', [@$JSON_OBJECT["foo"]["bar"] ?? "not found"]));
// (JSON_OBJECT?.foo?.bar ?? "not found") or (JSON_OBJECT?.["foo"]?.["bar"] ?? "not found"): "baz"

console_log(string_interpolation('(JSON_OBJECT?.foo?.baz ?? "not found") or (JSON_OBJECT?.["foo"]?.["baz"] ?? "not found"): ', [@$JSON_OBJECT["foo"]["baz"] ?? "not found"]));
// (JSON_OBJECT?.foo?.baz ?? "not found") or (JSON_OBJECT?.["foo"]?.["baz"] ?? "not found"): "not found"

console_log(string_interpolation('(JSON_OBJECT?.fruits?.[2] ?? "not found") or (JSON_OBJECT?.["fruits"]?.[2] ?? "not found"): ', [@$JSON_OBJECT["fruits"][2] ?? "not found"]));
// (JSON_OBJECT?.fruits?.[2] ?? "not found") or (JSON_OBJECT?.["fruits"]?.[2] ?? "not found"): "banana"

console_log(string_interpolation('(JSON_OBJECT?.fruits?.[5] ?? "not found") or (JSON_OBJECT?.["fruits"]?.[5] ?? "not found"): ', [@$JSON_OBJECT["fruits"][5] ?? "not found"]));
