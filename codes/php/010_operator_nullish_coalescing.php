...
console_log(string_interpolation('(JSON_OBJECT?.foo?.bar ?? "not found") or (JSON_OBJECT?.["foo"]?.["bar"] ?? "not found"): ', [(optional_chaining($JSON_OBJECT, "foo", "bar") ?? "not found")]));
// (JSON_OBJECT?.foo?.bar ?? "not found") or (JSON_OBJECT?.["foo"]?.["bar"] ?? "not found"): "baz"

console_log(string_interpolation('(JSON_OBJECT?.foo?.baz ?? "not found") or (JSON_OBJECT?.["foo"]?.["baz"] ?? "not found"): ', [(optional_chaining($JSON_OBJECT, "foo", "baz") ?? "not found")]));
// (JSON_OBJECT?.foo?.baz ?? "not found") or (JSON_OBJECT?.["foo"]?.["baz"] ?? "not found"): "not found"

console_log(string_interpolation('(JSON_OBJECT?.fruits?.[2] ?? "not found") or (JSON_OBJECT?.["fruits"]?.[2] ?? "not found"): ', [(optional_chaining($JSON_OBJECT, "fruits", 2) ?? "not found")]));
// (JSON_OBJECT?.fruits?.[2] ?? "not found") or (JSON_OBJECT?.["fruits"]?.[2] ?? "not found"): "banana"

console_log(string_interpolation('(JSON_OBJECT?.fruits?.[5] ?? "not found") or (JSON_OBJECT?.["fruits"]?.[5] ?? "not found"): ', [(optional_chaining($JSON_OBJECT, "fruits", 5) ?? "not found")]));
// (JSON_OBJECT?.fruits?.[5] ?? "not found") or (JSON_OBJECT?.["fruits"]?.[5] ?? "not found"): "not found"

console_log(string_interpolation('(JSON_OBJECT?.get_rectangle_area?.(7, 5) ?? "not found") or (JSON_OBJECT?.["get_rectangle_area"]?.(7, 5) ?? "not found"): ', [(optional_chaining(optional_chaining($JSON_OBJECT, "get_rectangle_area"), 7, 5) ?? "not found")]));
// (JSON_OBJECT?.get_rectangle_area?.(7, 5) ?? "not found") or (JSON_OBJECT?.["get_rectangle_area"]?.(7, 5) ?? "not found"): 35

console_log(string_interpolation('(JSON_OBJECT?.get_circle_area?.(7) ?? "not found") or (JSON_OBJECT?.["get_circle_area"]?.(7) ?? "not found"): ', [(optional_chaining(optional_chaining($JSON_OBJECT, "get_circle_area"), 7) ?? "not found")]));
// (JSON_OBJECT?.get_circle_area?.(7) ?? "not found") or (JSON_OBJECT?.["get_circle_area"]?.(7) ?? "not found"): 35

console_log(string_interpolation('(JSON_OBJECT?.get_rectangle_area?.(7, 5) ?? "not found") or (JSON_OBJECT?.["get_rectangle_area"]?.(7, 5) ?? "not found"): ', [(pipe(optional_chaining($JSON_OBJECT, "get_rectangle_area"), (fn($_) => (optional_chaining($_, 7, 5)))) ?? "not found")]));
// (JSON_OBJECT?.get_rectangle_area?.(7, 5) ?? "not found") or (JSON_OBJECT?.["get_rectangle_area"]?.(7, 5) ?? "not found"): 35

console_log(string_interpolation('(JSON_OBJECT?.get_circle_area?.(7) ?? "not found") or (JSON_OBJECT?.["get_circle_area"]?.(7) ?? "not found"): ', [(pipe(optional_chaining($JSON_OBJECT, "get_circle_area"), (fn($_) => (optional_chaining($_, 7)))) ?? "not found")]));
// (JSON_OBJECT?.get_circle_area?.(7) ?? "not found") or (JSON_OBJECT?.["get_circle_area"]?.(7) ?? "not found"): 35
