...
function pipe_v1(...$rest_arguments) {
    global $js_like_type;
    $pipe_last_result = null;
    $pipe_result = array_reduce($rest_arguments, (function ($current_result, $current_argument) use (&$pipe_last_result, $js_like_type) {
        $pipe_last_result = $current_result;
        if (get_type($current_result) === $js_like_type["Null"]) return $current_argument;
        if (get_type($current_argument) === $js_like_type["Function"]) return $current_argument($current_result);
        return null;
...
function pipe_v2(...$rest_arguments) {
    global $js_like_type;
    $pipe_last_result = null;
    $pipe_result = array_reduce_v2((function ($current_result, $current_argument) use (&$pipe_last_result, $js_like_type) {
        $pipe_last_result = $current_result;
...
pipe_v1(17, (fn($_) => (plus25($_))), (fn($_) => (multiply_by10($_))), (fn($_) => console_log($_))); // read from left to right

console_log(pipe_v1(17, (fn($_) => (plus25($_))), (fn($_) => (multiply_by10($_))))); // read from left to right

pipe_v2(17, (fn($_) => (plus25($_))), (fn($_) => (multiply_by10($_))), (fn($_) => console_log($_))); // read from left to right

console_log(pipe_v2(17, (fn($_) => (plus25($_))), (fn($_) => (multiply_by10($_))))); // read from left to right
