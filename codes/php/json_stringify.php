...
$json_stringify_v1 = fn($anything, $pretty = false) => (str_replace("{  }", "[object Function]", (str_replace("{}", "[object Function]", (($pretty === true) ? (str_replace("/\n$/", "", json_encode($anything, JSON_PRETTY_PRINT))) : (str_replace("{", "{ ", str_replace("}", " }", str_replace(",", ", ", str_replace(":", ": ", json_encode($anything))))))))))); // custom JSON.stringify() function json_stringify_v1

$json_stringify_v2 = fn($anything, $pretty = false) => $pipe((($pretty === true) ? ($pipe(json_encode($anything, JSON_PRETTY_PRINT), (fn($_) => str_replace("/\n$/", "", $_)))) : ($pipe(json_encode($anything), (fn($_) => str_replace(",", ", ", $_)), (fn($_) => str_replace("{", "{ ", $_)), (fn($_) => str_replace(":", ": ", $_)), (fn($_) => str_replace("}", " }", $_))))), (fn($_) => str_replace("{  }", "[object Function]", $_)), (fn($_) => str_replace("{}", "[object Function]", $_))); // custom JSON.stringify() function json_stringify_v2
...
$json_stringify_v3 = function ($anything, $pretty = false) use ($js_like_type, $get_type) {
    ...
    $json_stringify_inner = function ($anything_inner) use ($js_like_type, $get_type, &$json_stringify_inner) {
        $anything_inner_type = $get_type($anything_inner);
        if (($anything_inner_type === $js_like_type["Null"]) || ($anything_inner_type === $js_like_type["String"]) || ($anything_inner_type === $js_like_type["Numeric"]) || ($anything_inner_type === $js_like_type["Boolean"])) return $anything_inner;
        if ($anything_inner_type === $js_like_type["Object"]) {
            $object_entry_index = 0;
            $new_object = [];
            foreach ($anything_inner as $object_key => $object_value) {
