...
println("JSON_OBJECT?.foo?.bar or JSON_OBJECT?.['foo']?.['bar']: $(json_stringify(try JSON_OBJECT["foo"]["bar"] catch (any_error) nothing end))")
# JSON_OBJECT?.foo?.bar or JSON_OBJECT?.['foo']?.['bar']: "baz"

println("JSON_OBJECT?.foo?.baz or JSON_OBJECT?.['foo']?.['baz']: $(json_stringify(try JSON_OBJECT["foo"]["baz"] catch (any_error) nothing end))")
# JSON_OBJECT?.foo?.baz or JSON_OBJECT?.['foo']?.['baz']: null

println("JSON_OBJECT?.fruits?.[2] or JSON_OBJECT?.['fruits']?.[2]: $(json_stringify(try JSON_OBJECT["fruits"][2] catch (any_error) nothing end))")
# JSON_OBJECT?.fruits?.[2] or JSON_OBJECT?.['fruits']?.[2]: "mango"

println("JSON_OBJECT?.fruits?.[5] or JSON_OBJECT?.['fruits']?.[5]: $(json_stringify(try JSON_OBJECT["fruits"][5] catch (any_error) nothing end))")
# JSON_OBJECT?.fruits?.[5] or JSON_OBJECT?.['fruits']?.[5]: null

println("JSON_OBJECT?.get_rectangle_area?.(7, 5) or JSON_OBJECT?.[\"get_rectangle_area\"]?.(7, 5): $(json_stringify(try JSON_OBJECT["get_rectangle_area"](7, 5) catch (any_error) nothing end))")
# JSON_OBJECT?.get_rectangle_area?.(7, 5) or JSON_OBJECT?.[\"get_rectangle_area\"]?.(7, 5): 35

println("JSON_OBJECT?.get_circle_area?.(7) or JSON_OBJECT?.[\"get_circle_area\"]?.(7): $(json_stringify(try JSON_OBJECT["get_circle_area"](7) catch (any_error) nothing end))")
