...
function generate_number_sequence(start_number, stop_number)
    local number_sequence_array = {}
    if (stop_number > start_number) then
        for i = start_number, stop_number, 1 do
            table.insert(number_sequence_array, i)
        end
...
for i = 1, 10, 1 do -- start, stop, step
    if (i > 5) then break end
    if (i == 2) then goto next_iteration end
    console_log(string_interpolation("for loop ascending, i is ", {i}))
    ::next_iteration::
end

for i = 10, 1, -1 do -- start, stop, step
    if (i <= 5) then break end
    if (i == 9) then goto next_iteration end
    console_log(string_interpolation("for loop descending, i is ", {i}))
    ::next_iteration::
end

for array_item_index, i in ipairs(generate_number_sequence(1, 10)) do
    if (i > 5) then break end
    if (i == 2) then goto next_iteration end
...
for array_item_index, i in ipairs(generate_number_sequence(10, 1)) do
    if (i <= 5) then break end
    if (i == 9) then goto next_iteration end
