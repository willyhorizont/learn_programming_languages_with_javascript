...
console_log("\n-- JavaScript-like Array.reduce() in Lua table")
...
console_log(string_interpolation("total number: ", json_stringify(numbers_total, { ["pretty"] = true })))
-- total number: 41.2

console_log("\n-- JavaScript-like Array.reduce() in Lua table of hash-tables")
...
products_grouped = array_reduce(function (current_result, current_product)
        if (current_product["price"] > 100) then
            return spread_object(current_result, { ["expensive"] = spread_array(current_result["expensive"], { ["current_product"] = current_product }) })
        end
        return spread_object(current_result, { ["cheap"] = spread_array(current_result["cheap"], { ["current_product"] = current_product }) })
