...
function array_filter_v1(callback_function, any_array)
...
function array_filter_v2(callback_function, any_array)
    -- JavaScript-like Array.filter() function array_filter_v2
    local data_filtered = {}
    for array_item_index, array_item in ipairs(any_array) do
        if (callback_function(array_item, array_item_index, any_array) == true) then
...
console_log("\n-- JavaScript-like Array.filter() in Lua table")
...
console_log(string_interpolation("even numbers only: ", json_stringify(numbers_even)))
-- even numbers only: [12, 34, 36, 4, 254]

numbers_odd = array_filter_v1(function (number) return ((number % 2) ~= 0) end, numbers)
console_log(string_interpolation("odd numbers only: ", json_stringify(numbers_odd)))
...
console_log("\n-- JavaScript-like Array.filter() in Lua table of hash-tables")
...
products_below_100 = array_filter_v1(function (product) return (optional_chaining(function () return (product["price"]) end) <= 100) end, products)
...
products_above_100 = array_filter_v1(function (product) return (optional_chaining(function () return (product["price"]) end) > 100) end, products)
...
products_below_100 = array_filter_v2(function (product) return (optional_chaining(function () return (product["price"]) end) <= 100) end, products)
console_log(string_interpolation("products with price <= 100 only: ", json_stringify(products_below_100, { ["pretty"] = true })))
-- products with price <= 100 only: [
--     {
--         "code": "potato_chips",
...
products_above_100 = array_filter_v2(function (product) return (optional_chaining(function () return (product["price"]) end) > 100) end, products)
console_log(string_interpolation("products with price > 100 only: ", json_stringify(products_above_100, { ["pretty"] = true })))
-- products with price > 100 only: [
--     {
--         "code": "pasta",
