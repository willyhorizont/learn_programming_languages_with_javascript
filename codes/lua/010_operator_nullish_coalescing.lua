...
console_log('-- using JavaScript-like Nullish Coalescing Operator (??) function "nullish_coalescing"')

console_log(string_interpolation('(JSON_OBJECT?.foo?.bar ?? "not found") or (JSON_OBJECT?.["foo"]?.["bar"] ?? "not found"): ', json_stringify(nullish_coalescing(optional_chaining(function () return (JSON_OBJECT["foo"]["bar"]) end), "not found"))))
-- (JSON_OBJECT?.foo?.bar ?? "not found") or (JSON_OBJECT?.["foo"]?.["bar"] ?? "not found"): "baz"

console_log(string_interpolation('(JSON_OBJECT?.foo?.baz ?? "not found") or (JSON_OBJECT?.["foo"]?.["baz"] ?? "not found"): ', json_stringify(nullish_coalescing(optional_chaining(function () return (JSON_OBJECT["foo"]["baz"]) end), "not found"))))
-- (JSON_OBJECT?.foo?.baz ?? "not found") or (JSON_OBJECT?.["foo"]?.["baz"] ?? "not found"): null

console_log(string_interpolation('(JSON_OBJECT?.fruits?.[2] ?? "not found") or (JSON_OBJECT?.["fruits"]?.[2] ?? "not found"): ', json_stringify(nullish_coalescing(optional_chaining(function () return (JSON_OBJECT["fruits"][2]) end), "not found"))))
-- (JSON_OBJECT?.fruits?.[2] ?? "not found") or (JSON_OBJECT?.["fruits"]?.[2] ?? "not found"): "mango"

console_log(string_interpolation('(JSON_OBJECT?.fruits?.[5] ?? "not found") or (JSON_OBJECT?.["fruits"]?.[5] ?? "not found"): ', json_stringify(nullish_coalescing(optional_chaining(function () return (JSON_OBJECT["fruits"][5]) end), "not found"))))
-- (JSON_OBJECT?.fruits?.[5] ?? "not found") or (JSON_OBJECT?.["fruits"]?.[5] ?? "not found"): null

console_log(string_interpolation('(JSON_OBJECT?.get_rectangle_area?.(7, 5) ?? "not found") or (JSON_OBJECT?.["get_rectangle_area"]?.(7, 5) ?? "not found"): ', json_stringify(nullish_coalescing(optional_chaining(function () return (JSON_OBJECT["get_rectangle_area"](7, 5)) end), "not found"))))
-- (JSON_OBJECT?.get_rectangle_area?.(7, 5) ?? "not found") or (JSON_OBJECT?.["get_rectangle_area"]?.(7, 5) ?? "not found"): null

console_log(string_interpolation('(JSON_OBJECT?.get_circle_area?.(7) ?? "not found") or (JSON_OBJECT?.["get_circle_area"]?.(7) ?? "not found"): ', json_stringify(nullish_coalescing(optional_chaining(function () return (JSON_OBJECT["get_circle_area"](7)) end), "not found"))))
-- (JSON_OBJECT?.get_circle_area?.(7) ?? "not found") or (JSON_OBJECT?.["get_circle_area"]?.(7) ?? "not found"): null
