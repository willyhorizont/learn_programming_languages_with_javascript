...
print("\n(JSON_OBJECT?.foo?.bar ?? \"not found\"): ", JSON.pretty_generate(nullish_coalescing.call(JSON_OBJECT.dig("foo", "bar"), "not found"), { "indent": " " * 4 }))
# (JSON_OBJECT?.foo?.bar ?? "not found"): "baz"

print("\n(JSON_OBJECT?.foo?.baz ?? \"not found\"): ", JSON.pretty_generate(nullish_coalescing.call(JSON_OBJECT.dig("foo", "baz"), "not found"), { "indent": " " * 4 }))
# (JSON_OBJECT?.foo?.baz ?? "not found"): "not found"

print("\n(JSON_OBJECT?.fruits?.[2] ?? \"not found\"): ", JSON.pretty_generate(nullish_coalescing.call(JSON_OBJECT.dig("fruits", 2), "not found"), { "indent": " " * 4 }))
# (JSON_OBJECT?.fruits?.[2] ?? "not found"): "banana"

print("\n(JSON_OBJECT?.fruits?.[5] ?? \"not found\"): ", JSON.pretty_generate(nullish_coalescing.call(JSON_OBJECT.dig("fruits", 5), "not found"), { "indent": " " * 4 }))
