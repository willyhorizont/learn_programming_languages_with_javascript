...
for (my $i = 1; ($i <= 10); $i += 1) {
    last if ($i > 5);
    next if ($i == 2);
    console_log(string_interpolation("for loop v1, i is ", $i));
}

for my $i (1 .. 10) {
    last if ($i > 5);
    next if ($i == 2);
    console_log(string_interpolation("for loop v2 ascending v1, i is ", $i));
}

for my $i (@{generate_number_sequence(1, 10)}) {
    last if ($i > 5);
    next if ($i == 2);
    console_log(string_interpolation("for loop v2 ascending v2, i is ", $i));
}

for my $i (@{generate_number_sequence(10, 1)}) {
    last if ($i <= 5);
    next if ($i == 9);
    console_log(string_interpolation("for loop v2 descending, i is ", $i));
}

foreach my $i (1 .. 10) {
    last if ($i > 5);
    next if ($i == 2);
    console_log(string_interpolation("for loop v3 ascending v1, i is ", $i));
}

foreach my $i (@{generate_number_sequence(1, 10)}) {
    last if ($i > 5);
    next if ($i == 2);
    console_log(string_interpolation("for loop v3 ascending v2, i is ", $i));
}

foreach my $i (@{generate_number_sequence(10, 1)}) {
    last if ($i <= 5);
    next if ($i == 9);
    console_log(string_interpolation("for loop v3 descending, i is ", $i));
}
