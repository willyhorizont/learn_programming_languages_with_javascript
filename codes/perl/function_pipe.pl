...
console_log("# Pipe Function in Perl");

sub plus25 {
    my ($any_number) = @_;
    return ($any_number + 25);
}
...
pipe_v2(17, \&plus25, \&multiply_by10, \&console_log); # read from left to right

console_log(pipe_v2(17, \&plus25, \&multiply_by10)); # read from left to right

sub make_number_easy_to_say {
    my ($any_number) = @_;
    return (string_interpolation(int($any_number), ".something"));
}

sub get_circle_area_in_square_cm {
    my ($radius_in_cm) = @_;
    return (3.14 * ($radius_in_cm ** 2));
}

sub get_cylinder_volume_in_ml_or_cubic_cm {
    my ($circle_area_in_square_cm, $height_in_cm) = @_;
    return ($circle_area_in_square_cm * $height_in_cm);
}

sub get_mass_in_ml_or_cubic_cm {
    my ($volume_in_ml_or_cubic_cm, $density_in_gram_per_ml_or_cubic_cm) = @_;
