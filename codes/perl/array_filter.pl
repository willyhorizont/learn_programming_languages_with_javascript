...
sub array_filter_v2 {
    ...
    for (my $array_item_index = 0; ($array_item_index < scalar(@{$an_array_ref})); $array_item_index += 1) {
        my $array_item = $an_array_ref->[$array_item_index];
        my $is_condition_match = boolean($callback_function_ref->($array_item, $array_item_index, $an_array_ref));
        push(@{$new_array_ref}, $array_item) if ($is_condition_match eq "true");
    }
    return $new_array_ref;
}
...
sub array_filter_v3 {
    ...
    for (my $array_item_index = 0; ($array_item_index < scalar(@{$an_array_ref})); $array_item_index += 1) {
        my $array_item = $an_array_ref->[$array_item_index];
        push(@{$new_array_ref}, $array_item) if ((boolean($callback_function_ref->($array_item, $array_item_index, $an_array_ref))) eq "true");
    }
    return $new_array_ref;
}
...
{
    my $numbers_even_ref = array_filter_v1(sub { (([@_]->[0] % 2) == 0) }, $numbers_ref);
    console_log(string_interpolation("even numbers only: ", [$numbers_even_ref]));
    # even numbers only: [12, 34, 36, 4, 254]

    my $numbers_odd_ref = array_filter_v1(sub { (([@_]->[0] % 2) != 0) }, $numbers_ref);
    console_log(string_interpolation("odd numbers only: ", [$numbers_odd_ref]));
...
{
    my $numbers_even_ref = array_filter_v2(sub { (([@_]->[0] % 2) == 0) }, $numbers_ref);
    console_log(string_interpolation("even numbers only: ", [$numbers_even_ref]));
    # even numbers only: [12, 34, 36, 4, 254]

    my $numbers_odd_ref = array_filter_v2(sub { (([@_]->[0] % 2) != 0) }, $numbers_ref);
    console_log(string_interpolation("odd numbers only: ", [$numbers_odd_ref]));
    # odd numbers only: [27, 23, 65, 93, 87]
}
...
{
    my $numbers_even_ref = array_filter_v3(sub { (([@_]->[0] % 2) == 0) }, $numbers_ref);
    console_log(string_interpolation("even numbers only: ", [$numbers_even_ref]));
    # even numbers only: [12, 34, 36, 4, 254]

    my $numbers_odd_ref = array_filter_v3(sub { (([@_]->[0] % 2) != 0) }, $numbers_ref);
    console_log(string_interpolation("odd numbers only: ", [$numbers_odd_ref]));
    # odd numbers only: [27, 23, 65, 93, 87]
}

console_log('# using Perl Array.filter() built-in function "grep"');

{
    my $numbers_even_ref = [(grep { (($_ % 2) == 0) } @{$numbers_ref})];
    console_log(string_interpolation("even numbers only: ", [$numbers_even_ref]));
    # even numbers only: [12, 34, 36, 4, 254]

    my $numbers_odd_ref = [(grep { (($_ % 2) != 0) } @{$numbers_ref})];
    console_log(string_interpolation("odd numbers only: ", [$numbers_odd_ref]));
    # odd numbers only: [27, 23, 65, 93, 87]
}
...

{
    ...
    console_log(string_interpolation("products with price <= 100 only: ", json_stringify($products_below_100_ref, "pretty" => "true")));
    ...
    console_log(string_interpolation("products with price > 100 only: ", json_stringify($products_above_100_ref, "pretty" => "true")));
    # products with price > 100 only: [
    #     {
    #         "code": "pasta",
    #         "price": 321
...
{
    my $products_below_100_ref = [(grep { ((optional_chaining($_, "price")) <= 100) } @{$products_ref})];
    console_log(string_interpolation("products with price <= 100 only: ", json_stringify($products_below_100_ref, "pretty" => "true")));
    # products with price <= 100 only: [
    #     {
    #         "code": "potato_chips",
    ...
    my $products_above_100_ref = [(grep { ((optional_chaining($_, "price")) > 100) } @{$products_ref})];
    console_log(string_interpolation("products with price > 100 only: ", json_stringify($products_above_100_ref, "pretty" => "true")));
    # products with price > 100 only: [
    #     {
    #         "code": "pasta",
