...
sub factorial_v1 {
    my ($any_number) = @_;
    die("Argument should be a number") if (!is_triple_equal(get_type($any_number), throw_error_if_null($js_like_type_ref->{"Numeric"})));
    die("Argument should be >= 0") if ($any_number < 0);
    return 1 if ($any_number == 0);
    my $result = 1;
    my $i = $any_number;
    while (1 == 1) {
        $result *= $i;
