...
sub object_entries {
    ...
    my $new_array_ref = [];
    my $object_entry_index = 0;
    while (my ($object_key, $object_value) = each(%{$an_object_ref})) {
...
console_log('# using JavaScript-like Object.entries() function "object_entries"');

console_log(string_interpolation("friend_ref entries: ", [\@{object_entries(\%friend)}]));
console_log(string_interpolation("friend_ref entries: ", [object_entries({%friend})]));
...
console_log(string_interpolation("friend_ref entries: ", [\@{object_entries(\%{$friend_ref})}]));
console_log(string_interpolation("friend_ref entries: ", [object_entries($friend_ref)]));
# friend_ref entries: [["name", "Alisa"], ["country", "Finland"], ["age", 25]]
