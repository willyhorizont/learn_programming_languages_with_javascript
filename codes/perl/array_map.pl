...
@products_labeled = array_map_v1(sub { my ($product) = @_; +{spread_syntax_object(\%{$product}, {"label" => (($product->{"price"} > 100) ? "expensive" : "cheap")})} }, \@products);
print("labeled products: ", pretty_json_stringify(\@products_labeled), "\n");
# labeled products: [
#     {
#         "code": "pasta",
...
@products_labeled = array_map_v2(sub { my ($product) = @_; +{spread_syntax_object(\%{$product}, {"label" => (($product->{"price"} > 100) ? "expensive" : "cheap")})} }, \@products);
print("labeled products: ", pretty_json_stringify(\@products_labeled), "\n");
# labeled products: [
#     {
#         "code": "pasta",
