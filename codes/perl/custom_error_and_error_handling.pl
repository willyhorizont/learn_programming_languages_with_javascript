...
sub give_me_respect {
    my ($argument) = @_;
    # Custom Error
    die('Error: You should give me "respect"!') if (!is_triple_equal($argument, "respect"));
    return 'Thank you for giving me "respect"!';
}

try {
    my $response = give_me_respect("boo!");
    console_log($response);
} catch ($any_error_message) {
    console_log($any_error_message);
};
console_log("I'm sorry!");
