...
let countVowelsV1 = { (aString: String) -> Int in aString.split(separator: "").reduce(0) { (result: Int, letter: Substring) -> Int in ((["a", "i", "u", "e", "o", "A", "I", "U", "E", "O"].contains(letter)) ? (result + 1) : result) } }
print(countVowelsV1("Hello World")) // 3

let countVowelsV2 = { (aString: String) -> Int in aString.split(separator: "").reduce(0) { (result: Int, letter: Substring) -> Int in ((["A", "I", "U", "E", "O"].contains(letter.uppercased())) ? (result + 1) : result) } }
print(countVowelsV2("Hello World")) // 3

let countVowelsV3 = { (aString: String) -> Int in aString.split(separator: "").reduce(0) { (result: Int, letter: Substring) -> Int in (("aiueoAIUEO".contains(letter)) ? (result + 1) : result) } }
print(countVowelsV3("Hello World")) // 3

let countVowelsV4 = { (aString: String) -> Int in aString.split(separator: "").reduce(0) { (result: Int, letter: Substring) -> Int in (("AIUEO".contains(letter.uppercased())) ? (result + 1) : result) } }
