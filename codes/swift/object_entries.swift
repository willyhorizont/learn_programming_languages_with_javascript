...
// There's no JavaScript-like Object.entries() in Swift.
// But, we can create our own function to mimic it in Swift.

func objectEntries(_ anObject: MyObject) -> MyArray {
    // JavaScript-like Object.entries() function
    var newArray = MyArray()
    for (_, (objectKey, objectValue)) in friend.enumerated() {
        newArray.append([objectKey, objectValue] as MyArray)
    }
    return newArray
}

print("\n// JavaScript-like Object.entries() in Swift Dictionary")
...
print("friend entries: \(jsonStringify(Array(objectEntries(friend)) as MyArray))")
