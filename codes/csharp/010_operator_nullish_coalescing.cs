...
class Program {
    static void Main(string[] Args) {
        ...
        Console.WriteLine($"(JSON_OBJECT?.foo?.bar ?? 'not found') or (JSON_OBJECT?.['foo']?.['bar'] ?? 'not found'): {JsonStringify(OptionalChainingV2(JSON_OBJECT, "foo", "bar") ?? "not found")}");
        // (JSON_OBJECT?.foo?.bar ?? 'not found') or (JSON_OBJECT?.['foo']?.['bar'] ?? 'not found'): "baz"

        Console.WriteLine($"(JSON_OBJECT?.foo?.baz ?? 'not found') or (JSON_OBJECT?.['foo']?.['baz'] ?? 'not found'): {JsonStringify(OptionalChainingV2(JSON_OBJECT, "foo", "baz") ?? "not found")}");
        // (JSON_OBJECT?.foo?.baz ?? 'not found') or (JSON_OBJECT?.['foo']?.['baz'] ?? 'not found'): "not found"

        Console.WriteLine($"(JSON_OBJECT?.fruits?.[2] ?? 'not found') or (JSON_OBJECT?.['fruits']?.[2] ?? 'not found'): {JsonStringify(OptionalChainingV2(JSON_OBJECT, "fruits", 2) ?? "not found")}");
        // (JSON_OBJECT?.fruits?.[2] ?? 'not found') or (JSON_OBJECT?.['fruits']?.[2] ?? 'not found'): "banana"

        Console.WriteLine($"(JSON_OBJECT?.fruits?.[5] ?? 'not found') or (JSON_OBJECT?.['fruits']?.[5] ?? 'not found'): {JsonStringify(OptionalChainingV2(JSON_OBJECT, "fruits", 5) ?? "not found")}");
