...
# There's no JavaScript-like Object.entries() in R.
...
object_entries <- function(an_object) {
    ...
    for (object_entry_index in seq_along(an_object)) {
        ...
        new_array <- append(new_array, list(list(object_key, object_value)))
    }
    return(new_array)
}

cat("\n# JavaScript-like Object.entries() in R Associative-list\n")
...
cat("# using JavaScript-like Object.entries() function \"object_entries\"\n")

cat(paste(sep = "", "friend entries: ", json_stringify(object_entries(friend)), "\n"))
