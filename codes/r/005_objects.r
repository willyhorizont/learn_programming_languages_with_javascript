...
deliveryResponse <- list(
    deliveryResponseKeyMessage = "ok" # this is not working in R, the result is `{"deliveryResponseKeyMessage": "ok"}`
)
deliveryResponse <- list(
    message = "ok"
)
cat(paste(sep = "", "deliveryResponse: ", jsonStringify(deliveryResponse, pretty = TRUE), "\n"))
