def MyObject(anObject: Any): Map[String, Any] = if (anObject.asInstanceOf[Map[String, Any]].size == 0) Map.empty[String, Any].asInstanceOf[Map[String, Any]] else anObject.asInstanceOf[Map[String, Any]]
def MyArray(arrayItems: Any*): Array[Any] = if (arrayItems.toArray.asInstanceOf[Array[Any]].length == 0) Array.empty[Any].asInstanceOf[Array[Any]] else arrayItems.toArray.asInstanceOf[Array[Any]]
...
def jsonStringify(anything: Any = null, indent: String = "    ", pretty: Boolean = false): String = {
    ...
    def jsonStringifyInner(anythingInner: Any, indentInner: String, prettyInner: Boolean): String = {
        ...
        if (anythingInner.isInstanceOf[Map[_, _]]) {
            ...
            indentLevel += 1
            var result: String = s"{${if (pretty == true) s"\n${indentInner * indentLevel}" else ""}"
            for (((objectKey, objectValue), objectEntryIndex) <- anythingInner.asInstanceOf[Map[String, Any]].toArray.zipWithIndex) {
...
val arrayToObject = new Function1[Array[Any], Map[String, Any]] {
    def apply(anArray: Array[Any]): Map[String, Any] = {
        var newObject: Map[String, Any] = Map.empty[String, Any]
...
val countryCapitalsInAsia = MyObject(Map(
    "Thailand" -> "Bangkok",
    "China" -> "Beijing",
    "Japan" -> "Tokyo"
))
...
val countryCapitalsInEurope = MyObject(Map(
    "France" -> "Paris",
    "England" -> "London"
))
...
val combination3 = (MyObject(Map()) ++ (countryCapitalsInAsia ++ countryCapitalsInEurope))
...
val combination4 = (MyObject(Map()) ++ (countryCapitalsInAsia ++ MyObject(Map("Germany" -> "Berlin", "Italy" -> "Rome"))))
...
val combination8 = (MyArray() ++ (fruits :+ MyObject(Map("Germany" -> "Berlin", "Italy" -> "Rome"))))
...
val combination9 = (MyObject(Map()) ++ (countryCapitalsInAsia ++ MyObject(Map("countryCapitalsInEurope" -> countryCapitalsInEurope))))
...
val combination10 = (MyObject(Map()) ++ (countryCapitalsInAsia ++ MyObject(Map("countryCapitalsInEurope" -> MyObject(Map("Germany" -> "Berlin", "Italy" -> "Rome"))))))
...
val combination11 = (MyObject(Map()) ++ (countryCapitalsInAsia ++ MyObject(Map("vegetables" -> vegetables))))
...
val combination12 = (MyObject(Map()) ++ (countryCapitalsInAsia ++ MyObject(Map("vegetables" -> MyArray("Cucumber", "Cabbage")))))
...
val combination13 = (MyObject(Map()) ++ (countryCapitalsInAsia ++ arrayToObject(vegetables)))
...
val combination14 = (MyObject(Map()) ++ (countryCapitalsInAsia ++ arrayToObject(MyArray("Cucumber", "Cabbage"))))
...
// this combination throw an error in JavaScript
// val combinationErrorInJavascript2 = (MyArray() ++ (fruits :+ MyObject(Map("Germany" -> "Berlin", "Italy" -> "Rome"))))
